...
setwd("/Users/kuanjiaen/Documents/ST2195/dataverse_files")
...
carriers_df <- read.csv("carriers.csv", header = TRUE)   # load in data to dataframe
planes_df <- read.csv("plane-data.csv", header = TRUE)
dbWriteTable(conn, "airports", airports_df)
dbWriteTable(conn, "carriers", carriers_df)     # create table name
dbWriteTable(conn, "planes", planes_df)

for (i in c(2000:2005)) {
  ontime_df_temp <- read.csv(paste0(i, ".csv"),header = TRUE)
  if (i == 2000) {
    dbWriteTable(conn, "ontime", ontime_df_temp)
  } else {
    dbWriteTable(conn, "ontime", ontime_df_temp, append = TRUE)
  }
}
...
"SELECT carriers.Description AS carrier, COUNT(*) AS total
 ...
 ORDER by total DESC")
print(paste(q3[1, "carrier"], "has the highest number of inbound flights"))
